...
redef class AAttrPropdef
	...
	do
		...
		if mwritepropdef != null and not annotation_no_contract.is_empty then
			mwritepropdef.no_contract_apply(v, create_set_nsignature)
			return
		end
		...
		if not annotation_expect.is_empty then
			...
			mwritepropdef.construct_contract(v, self.create_set_nsignature, annotation_expect, mwritepropdef.mproperty.build_expect, exist_set_contract)
		end
		...
		if not annotation_ensure.is_empty then
			...
			mwritepropdef.construct_contract(v, self.create_set_nsignature, annotation_ensure, mwritepropdef.mproperty.build_ensure, exist_set_contract)
		end
	end
	...
	do
		...
		if propdef != null and propdef.mclassdef.mclass.check_invariant then
			...
			if mwritepropdef != null then mwritepropdef.mproperty.define_invariant_facet(v, mwritepropdef.msignature.as(not null), create_set_nsignature, propdef.mclassdef, false, minvariant)
		end
	end
